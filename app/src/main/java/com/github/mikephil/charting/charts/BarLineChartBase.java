...
public abstract class BarLineChartBase<T extends BarLineScatterCandleData<? extends BarLineScatterCandleRadarDataSet<? extends Entry>>>
    ...
    protected void calculateOffsets() {
        ...
        float min = Utils.convertDpToPixel(0f);
        ...
        if (mLegend != null) {
            ...
            // them
            mLegend.setOffsetTop(0);
            mLegend.setOffsetLeft(0);
        }
    ...
    public synchronized void centerViewPort(final float xIndex, final float yVal) {
        ...
        final ChartInterface chartInterface = this;
        ...
        (new Thread(){
            ...
            public void run(){
                ...
                final float[] pts = new float[] { xIndex - indicesInView / 2f, yVal + valsInView / 2f };
                handler.post(new Runnable(){
                    public void run() {
                        mTrans.centerViewPort(pts, chartInterface);
    ...
    // Custom methods
    public float getVisibleDomainCount(){
        float indicesInView = mDeltaX / mTrans.getScaleX();
        return indicesInView;
    }
}
